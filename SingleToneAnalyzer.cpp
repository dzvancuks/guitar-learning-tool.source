...
bool SingleToneAnalyzer::find_pitch(std::vector<float>& input, double& freq, double& logPower){
    ...
    indexOfMax = findNSDFsubMaximum(autocorr_buf, k, 0.8); // TODO fix koeficients; this part must be expanded to match Tartini algorithm (MyTransforms::calculateAnalysisData)
    ...
    if (y > 0.6) // if correlation is > 60%
      	if(logPower > noiseFloor + 5) // TODO tapping has low noise floor. Make option-enabled value
            return true;
        else
            return false;
    else 
