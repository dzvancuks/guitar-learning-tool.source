...
{
    pitch = npitch; // pitch = 0 => pauze (type/length is defined by nlength)
}

SingleNote::SingleNote(const SingleNote& orig) {
    //TODO
}
...
void SingleNote::AdjustNote(QPointF point, int clef) { // TODO make BAR do the tails
    //qreal thrd_ln = point.y() + 2 * VISUAL_NOTE_VSPACING;
    qreal n;
    int   in, inc;
...
#define A 220.0
    ...
    inc = 0;
    if (in >= 0) {
        for(int i = 0; i < in; i++) {                                  // skip every b and # half-tone for pos startinf from f
            int is = i % 12;
        ...
        for(int i = 0; i > in; i--) {                                  // skip every half-tone for pos startinf from f
            int is = i % 12;
    ...
    //if (y() > thrd_ln) tail_direction = true;
...
void SingleNote::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    ...
    // Tail's "pole"
    // Moved to Bar, to be able to stich tails
