...
void Carriage::RecalculateConstants() {
    VisualSizeSpeed etalon_vss = GlobalGetVisualSizeSpeed(etalon, 0);
    C1 = ((VISUAL_NOTE_SPACING*etalon_vss.size)/(TICS*etalon_vss.speed))/CA; // constants for carriage speed
    C2 = ((VISUAL_SIZE*etalon_vss.size)/(TICS*etalon_vss.speed))/CA;
    
    visual_speed = 1;    //reset start speed
    v_est = visual_speed/CA; // Must be x/CA on init due to X*ta*bpm    // Estimated speed (before timer lag adjustment): note speed * note length per 1 TICK
}
...
void Carriage::advance(int step)
 {
    ...
    foreach (QGraphicsItem* item, collided_items) {
                ...
                if (!repeat_border.isEmpty() && repeat_border.top() == bar) {
                    ...
                    if (--repeat_count.top() <= 1) {
                        ...
                        if (repeat_start.top()->GetId() != 1) { // 1-st bar is always repeatable; repeat_start_ignore removes curr bar infinite looping
                            ...
                            //next_bar_repeat_1 = NULL; // TODO test repeat endings! Dunno why I added this line, but on B1-[1_B2:|-[2_B3-... NRE didn't work
                        }
                    }
                } else if (!bar->IsLast()) {
        ...
        if (!timer.isActive()) {
            qreal timeout = (60000.0/bpm) * ((qreal)note->GetLength()/etalon); //TODO use Continued flag to extent timeout till next bar;
            emit PassNote(note);
