...
void Track::AddBarAt(Bar* bar, unsigned pos) { 
    unsigned i;
    std::vector<Bar*>::iterator it = bars.begin();

    bar->SetClef(clef);
    if (pos > 0) {
        for (i = 0; i < pos-1; i++, it++);
...
void Track::RemoveBarAt(unsigned pos) {
    ...
    for (i = 0; i < pos-1; i++, it++);
    bars.at(i)->ClearNeighbour();
    bars.erase(it);                                 // TODO prev/next pointers
}
